...
module.exports = class Tunnel extends EventEmitter {
  constructor(opts = {}) {
    ...
    this.opts = opts;
    this.closed = false;
    this.allowInsecureSslRequests = opts.allow_insecure_ssl_requests;
    if (!this.opts.host) {
  ...
  _getInfo(body) {
    ...
    const { local_https, local_cert, local_key, local_ca, allow_invalid_cert, allow_insecure_ssl_requests } = this.opts;
    return {
      name: id,
      url,
      cached_url,
  ...
  _init(cb) {
    ...
    if (this.allowInsecureSslRequests)
    ...
    // where to quest
    const uri = baseUri + (assignedDomain || opt.tunnelId ? `?new&tunnelId=${opt.tunnelId}` : '?new');
